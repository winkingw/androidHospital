...
public class MockDataUtil {
    ...
    private static void insertAppointments(DBHelper dbHelper) {
        ...
        for (String[] row : data) {
            ...
            long sourceId = findSourceIdByDoctor(db, doctorId);
            if (sourceId <= 0) {
                continue;
            }
     ...
     * 查询指定医生的第一个可用号源
     */
    private static long findSourceIdByDoctor(SQLiteDatabase db, long doctorId) {
        Cursor c = db.rawQuery(
                "SELECT rs.id FROM t_register_source rs "
                + "JOIN t_doctor_schedule ds ON rs.schedule_id = ds.id "
                + "WHERE ds.doctor_id = ? ORDER BY ds.id, rs.slot_start_time LIMIT 1",
