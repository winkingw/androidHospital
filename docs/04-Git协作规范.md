...
| 组员 | feature 分支 | 负责模块 |
|------|------------|---------|
| 组员A | `feature/01-home` | 首页 + 公共模块（数据库/工具类/模拟数据） |
| 组员B | `feature/02-health-admin` | 健康档案 + 后台管理 |
| 组员C | `feature/03-profile` | 个人中心 + 缴费 + 二维码 + 医保卡 |

> 完整功能编号分配以 **07-任务分工表.md** 为准，功能总表见 **《医院线上挂号预约 APP 完整功能总表.docx》**。

### 防冲突规则

- 各队员只修改自己 `feature/` 分支下的代码
- 公共模块（`bean/`, `db/`, `util/`）由组长或队员A先提交，其他队员只读不写
- 跨组页面入口（如功能5的缴费入口在首页）通过 Intent 跳转，不直接共享 Activity 实例
